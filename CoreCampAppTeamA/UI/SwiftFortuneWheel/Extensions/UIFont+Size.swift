...
extension SFWFont {
    ...
    func number(ofCharacters text: String, thatFit width: CGFloat, numberOfLines: Int = 1) -> Int {
        let fontRef = CTFontCreateWithName(fontName as CFString, pointSize, nil)
        ...
        CTFramesetterSuggestFrameSizeWithConstraints(frameSetterRef, CFRangeMake(0, 0), nil, CGSize(width: width, height: CGFloat(numberOfLines) * lineHeight), &characterFitRange)
